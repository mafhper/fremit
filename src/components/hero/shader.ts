...
export const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Cosmic Night Scene (Stars & Bloom)
    if (uStars > 0.01) {
      float starSeed = hash(floor(uv * 420.0));
      float star = step(0.996, starSeed);
      star *= smoothstep(0.2, 1.0, hash(floor(uv * 420.0) + 13.7));
      star *= (0.5 + 0.5 * sin(uTime * 2.0 + starSeed * 62.8)); // Twinkle
      base += star * uStars * mix(uColorB, uColorC, starSeed);
    }

    if (uBloom > 0.01) {
      float burst = pow(smoothstep(0.62, 0.0, distance(uv, uCenter)), 4.0);
      base += burst * uBloom * uColorB * 0.6;
    }
